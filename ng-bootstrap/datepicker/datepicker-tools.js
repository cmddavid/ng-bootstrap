...
function isDateSelectable(date, minDate, maxDate, isDisabled, markDisabled) {
    // clang-format off
    return !(!(0, _util.isDefined)(date) || isDisabled || markDisabled && markDisabled(date, { year: date.year, month: date.month }) || minDate && date.before(minDate) || maxDate && date.after(maxDate));
    // clang-format on
}
...
function getFirstViewDate(calendar, date, firstDayOfWeek) {
    ...
    var today = new _ngbDate.NgbDate(date.year, date.month, date.day);
    ...
    while (!reachedTheFirstDayOfTheLastWeekOfPreviousMonth() && !firstDayOfCurrentMonthIsAlsoFirstDayOfWeek()) {
        today = new _ngbDate.NgbDate(yesterday.year, yesterday.month, yesterday.day);
